...
ROITimeCourseFileName = os.path.join(dataDir, 'ROIdata', 'DICS_ROITimeCourses.npy')
ROITimeCourseDataFrameFileName = os.path.join(dataDir, 'ROIdata', 'DICS_ROITimeCourses.csv')
...
for subjectID in subjectIDs:
    ...
    for i in np.arange(len(ROIs)):
        df = pd.DataFrame.from_dict({'Age': ages[counter], 'Current': label_ts[i,:]})
...
ROIData = np.asarray(allData)
np.save(ROITimeCourseFileName, ROIData)

one_df.to_csv(ROITimeCourseDataFrameFileName)
